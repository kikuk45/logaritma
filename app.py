...
# -----------------------------------------------------------------------------
# FUNGSI UNTUK MEMUAT BACKGROUND PNG LOKAL & ADAPTIF
# -----------------------------------------------------------------------------
def set_adaptive_background(image_file):
    ...
    if img_path.exists():
        ...
        # Menggunakan PNG lokal dengan overlay semi-transparan agar teks tetap kontras
        background_css = f"""
        background-image: linear-gradient(rgba(255, 255, 255, 0.88), rgba(255, 255, 255, 0.88)), url("data:image/png;base64,{encoded_string}") !important;
        ...
        # Fallback pola geometri SVG jika file PNG belum tersedia di repository
        svg_pattern = '''
SVG

'''
encoded_svg = base64.b64encode(svg_pattern.encode("utf-8")).decode()
background_css = f"""
...
Panggil fungsi background dengan nama file PNG Anda
set_adaptive_background("bg_siswa.png")
